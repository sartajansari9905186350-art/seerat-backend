...
async function run() {
  ...
  const unauthPostDelData = await unauthorizedPostDelRes.json() as any;
  ...
  const authPostDelData = await authorizedPostDelRes.json() as any;
  ...
  const reelData = await createReelRes.json() as any;
  if (!reelData.success) {
  ...
  const unauthReelDelData = await unauthorizedReelDelRes.json() as any;
  ...
  const authReelDelData = await authorizedReelDelRes.json() as any;
